...
function B2BPage() {
  ...
  const cols: Column<B2BProduct>[] = [
    ...
    {
      key: "sub",
      header: "B2B Sub Category",
      render: (p) => <span className="text-sm">{p.subCategory}</span>,
    },
    {
      key: "material",
      header: "Material",
      render: (p) => <span className="text-xs text-muted-foreground">{p.material}</span>,
    },
    {
      key: "sample",
      header: "Sample",
  ...
  return (
    <PageShell
      title="B2B Shop Products"
      subtitle="Bulk & B2B catalog management"
      actions={
        <Button onClick={openNew}>
          <Plus className="mr-1 h-4 w-4" /> Add B2B Product
        </Button>
      }
    >
      <DataTable
        rows={data}
        columns={cols}
        searchKeys={["code", "name", "subCategory", "material"]}
        onExport={() => {
          exportCsv("arreniux-b2b.csv", data);
