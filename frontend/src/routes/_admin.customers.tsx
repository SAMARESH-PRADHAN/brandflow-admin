...
function CustomersPage() {
  ...
  const cols: Column<Customer>[] = [
    ...
    },
    {
      key: "address",
      header: "Address",
      render: (c) => <span className="line-clamp-1 max-w-[220px] text-xs">{c.address}</span>,
    },
    {
      key: "orders",
      header: "Orders",
      render: (c) => <span className="num text-sm">{c.totalOrders}</span>,
      className: "text-right",
      sortable: true,
      getValue: (c) => c.totalOrders,
    },
    {
      key: "spend",
      header: "Total Spend",
      render: (c) => <span className="num text-sm font-semibold">{inrFull(c.totalSpend)}</span>,
      className: "text-right",
      sortable: true,
      getValue: (c) => c.totalSpend,
    },
    {
      key: "join",
      header: "Join Date",
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  mutationError(err, editing ? "Failed to update customer" : "Failed to add customer");
                }
              }}
            >
              {editing ? "Save" : "Add"}
