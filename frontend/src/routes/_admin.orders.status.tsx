...
import { generateShippingSlipPDF } from "@/lib/pdf";
import { Download } from "lucide-react"; // add Download to your existing lucide-react import line instead if you prefer one import
    ...
    <PageShell title="Order Updation" subtitle="Kanban flow — click a card to preview details">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {STATUSES.map((s) => (
          <div
            key={s}
            className="flex min-h-[400px] flex-col rounded-2xl border border-border bg-secondary/30 p-3"
          >
            <div
              className={`mb-3 inline-flex items-center justify-between rounded-xl border px-3 py-2 ${COLORS[s]}`}
            >
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className="num text-xs font-semibold">
                      {inrFull(o.totalAmount > 0 ? o.totalAmount : o.qty * o.unitPrice)}
                    </span>
                  </div>
                  ...
                  <div className="mt-2 flex flex-col gap-1.5">
                    ...
                    {s === "Shipped" && (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          generateShippingSlipPDF(o);
                          toast.success("Shipping slip downloaded");
                        }}
                      >
                        <Download className="mr-1 h-3 w-3" /> Download Shipping Slip
                      </Button>
                    )}
                  </div>
                </button>
      ...
      <SectionCard title="Status legend" subtitle="Every status has its own semantic color">
        <div className="flex flex-wrap gap-2">
          {STATUSES.map((s) => (
            <span
              key={s}
              className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${COLORS[s]}`}
            >
              {s}
            </span>
          ))}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border p-3">
                ...
                <div className="text-xs text-muted-foreground">
                  {preview.category} • {preview.subCategory} • {preview.material}
                </div>
                <div className="mt-1 text-xs">
                  Qty <b>{preview.qty}</b> • {preview.printType} @ {preview.printLocation}
                </div>
              </div>
              ...
              <div className="flex justify-end gap-2">
                <Button asChild variant="outline">
                  <Link to={`/orders/${preview.id}`}>
                    <ExternalLink className="mr-1 h-4 w-4" /> Full details
                  </Link>
                </Button>
                {preview.status === "Shipped" && (
                  <Button
                    variant="outline"
                    onClick={() => {
                      generateShippingSlipPDF(preview);
                      toast.success("Shipping slip downloaded");
                    }}
                  >
                    <Download className="mr-1 h-4 w-4" /> Shipping Slip
                  </Button>
                )}
