// Deterministic demo data — mirrors frontend/src/lib/store.ts seed generators.
...
function seedOrders(products: Product[], customers: Customer[]): Order[] {
  return Array.from({ length: 100 }, (_, i) => {
    ...
    return {
      ...
      printType: pick(PRINT_TYPES),
      printLocation: pick(PRINT_LOCATIONS),
...
function seedAgentVisits(agents: Agent[]): AgentVisit[] {
  ...
  const reqs = [
    "500 corporate polo t-shirts",
    "Hotel staff uniform kit — 200 pcs",
    "Bulk hoodies for onboarding",
    "Custom aprons for restaurant chain",
    "Full welcome kit for new hires",
  ];
...
function seedNotifications(orders: Order[], products: Product[]): Notification[] {
  ...
  list.push({
    id: "NTF-9000",
    type: "system",
    title: "Welcome to Arreniux Admin",
...
export function generateDemoData() {
  ...
  const sampleOrders = seedSampleOrders(products, customers);
  const allOrders = [...orders, ...sampleOrders];

  return {
    products,
    b2bProducts: seedB2B(),
    newCollection: seedNewCollection(),
    welcomeKits: seedWelcomeKits(),
    customers,
    agents,
    agentVisits: seedAgentVisits(agents),
    notifications: seedNotifications(orders, products),
    orders: allOrders,
    payments: seedPayments(orders),
    reviews: seedReviews(products, customers),
